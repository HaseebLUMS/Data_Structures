...
void test_delete(BST<int>* tree)
{
	...
	for (int i=0;i<test_vec.size();i++)
	{
		if (test_vec[i]==ref_vec[i])
		{
			...
			//cout << test_vec[i] << " " << ref_vec[i] << endl;
		}
...
void test_height(BST<int>* tree)
{
	...
	cout<< "BALANCE FACTOR : " <<tree->balanceFactor(tree->getRoot())<<endl;
	cout<<endl;
	print(tree->getRoot());
	node<int>* p= tree->getRoot();
	//p = tree->rotateleft(tree->getRoot());
	p = tree->balance(tree->getRoot());
	//p = tree->balance(p);
	//p = tree->balance(p);
	cout<<endl<<endl;
